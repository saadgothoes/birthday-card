...
<head>
    ...
    * {
    ...
    /* Step 9 — the clip picker.

       The story takes two minutes of a song, the way an Instagram story takes
       a slice: not a start and an end to line up, just a fixed-length window
       slid along the track until it sits over the right part. A single range
       input does the whole job — its thumb is widened to the length it covers,
       so the thing being dragged *is* the selection, and the travel of a range
       input (rail minus thumb) is already exactly the travel of the window. */
    .music-clip {
        margin-top: 1.4rem;
    ...
    .music-clip-head strong {
        display: block;
        font-size: 0.92rem;
    }

    .music-clip-head .field-hint {
        margin-top: 0.2rem;
    }
    ...
    .music-clip-slider input[type="range"] {
        position: absolute;
        top: 0;
        left: 0;
        width: 100%;
    ...
    /* --clip-window is the clip's share of the track, in pixels, set from
       the song's length once it is known. */
    .music-clip-slider input[type="range"]::-webkit-slider-thumb {
        -webkit-appearance: none;
        width: var(--clip-window, 60px);
        height: 26px;
        border-radius: 9px;
        background: var(--accent);
    ...
    .music-clip-slider input[type="range"]::-moz-range-thumb {
        width: var(--clip-window, 60px);
        height: 26px;
        border-radius: 9px;
        background: var(--accent);
    ...
    .music-clip-slider input[type="range"]:active::-webkit-slider-thumb {
        cursor: grabbing;
    }

    .music-clip-slider input[type="range"]::-moz-range-track {
        background: transparent;
        border: none;
    }

    .music-clip-slider input[type="range"]:focus-visible::-webkit-slider-thumb {
        box-shadow: 0 0 0 4px var(--accent-soft);
    }

    .music-clip-slider input[type="range"]:disabled::-webkit-slider-thumb {
        opacity: .55;
        cursor: default;
    }
    ...
    .music-clip-vol input[type="range"] {
        width: 84px;
        accent-color: var(--accent);
        cursor: pointer;
    }

    .music-clip .field-hint {
        margin-top: 0.55rem;
    }
    ...
    /* ── Subscription gate on the QR step ────────────────────── */
    .sub-gate {
    ...
    /* ── Plan strip in the sidebar ───────────────────────────── */
    .plan-strip {
        margin: 0 1.1rem 1rem;
        padding: .85rem .95rem;
        border-radius: 12px;
        background: var(--surface-2, #f8f7fc);
        border: 1.5px solid var(--border, #e7e0fa);
    }
    ...
    .plan-strip .plan-name {
        font-size: .85rem;
        font-weight: 700;
        margin-bottom: .45rem;
    }

    .plan-strip .plan-usage {
        font-size: .75rem;
        color: var(--text-muted, #6b6478);
        display: flex;
        justify-content: space-between;
    ...
    .plan-strip .plan-usage b {
        color: var(--text, #120d1c);
    }
    ...
    .draft-save-btn.solid {
        background: var(--accent, #8B5CF6);
        border-color: var(--accent, #8B5CF6);
        color: #fff;
    }

    .draft-save-btn.solid:hover {
    ...
    .draft-before-qr {
        ...
        gap: 1.2rem;
        flex-wrap: wrap;
        border: 1.5px dashed var(--border, #e7e0fa);
        border-radius: 14px;
        padding: 1.1rem 1.3rem;
        margin-bottom: 1.4rem;
        background: var(--surface-2, #faf9ff);
    }

    .draft-before-qr strong {
        font-size: .9rem;
    }
    ...
    @media (max-width: 768px) {
        ...
        .draft-save-btn.solid::after {
            content: '';
        }
    }

    
    /* ── Main Dashboard link, top of the sidebar ─────────────── */
    .main-dash-link {
        display: flex;
        align-items: center;
        gap: .7rem;
    ...
    .main-dash-link:hover {
        border-color: var(--accent, #8B5CF6);
        background: var(--accent-soft, #f3edfe);
    ...
    .main-dash-link:hover .main-dash-icon {
        background: var(--accent, #8B5CF6);
        color: #fff;
    }

    .main-dash-text strong {
        display: block;
        font-size: .85rem;
        font-weight: 700;
        line-height: 1.2;
...
<body>
    ...
    <aside class="sidebar">
        ...
        <a href="{{ route('client.cards') }}" class="main-dash-link">
            <span class="main-dash-icon">
            ...
            <span class="main-dash-text">
                <strong>Main Dashboard</strong>
                <small>All cards &amp; drafts</small>
            </span>
        </a>
        ...
        <div class="plan-strip">
            ...
            <div class="plan-usage"><span>Allowed</span><b>{{ $cardLimit }}</b></div>
            <div class="plan-usage"><span>Created</span><b>{{ $cardsUsed }}</b></div>
            <div class="plan-usage"><span>Remaining</span><b>{{ $cardsRemaining }}</b></div>
        </div>
        ...
        <nav class="nav-steps">
            ...
            <div class="step-item" onclick="goToStep(9)">
                ...
                <div>
                    ...
                    <div class="step-sub">Choose a birthday song</div>
                </div>
            </div>
    ...
    <main class="main">
        <div class="topbar">
            ...
            <div class="topbar-right">
                <button class="draft-save-btn" onclick="openDraftModal()" title="Name this card and save it">
                    💾 Save as Draft
                </button>
                ...
                <div class="user-dropdown">
                    ...
                    <div class="user-dropdown-menu" id="userDropdown">
                        <a href="{{ route('client.cards') }}" class="dropdown-item">
        ...
        $showPaymentPopup = $daysLeft !== null && $daysLeft <= 6 && Auth::user()->status === 'active';
                            ...
                            <div class="variant-grid">
                                <div class="variant-choice" id="boyVariant1" onclick="selectVariant(1)">
                                    ...
                                    <div class="variant-thumb">
                                        <iframe data-src="{{ route('boy.page.variant',['page'=>1,'variant'=>1]) }}"
                                            tabindex="-1"></iframe>
                                    </div>
                                ...
                                <div class="variant-choice" id="boyVariant2" onclick="selectVariant(2)">
                                    <div class="variant-check">✓</div>
                                    <div class="variant-thumb">
                                        <iframe data-src="{{ route('boy.page.variant',['page'=>1,'variant'=>2]) }}"
                                            tabindex="-1"></iframe>
                                    </div>
                        ...
                        <div class="variant-section" id="girlVariants">
                            ...
                            <div class="variant-grid">
                                <div class="variant-choice" id="girlVariant1" onclick="selectVariant(1)">
                                    <div class="variant-check">✓</div>
                                    <div class="variant-thumb">
                                        <iframe data-src="{{ route('girl.page.variant',['page'=>1,'variant'=>1]) }}"
                                            tabindex="-1"></iframe>
                                    </div>
                                ...
                                <div class="variant-choice" id="girlVariant2" onclick="selectVariant(2)">
                                    <div class="variant-check">✓</div>
                                    <div class="variant-thumb">
                                        <iframe data-src="{{ route('girl.page.variant',['page'=>1,'variant'=>2]) }}"
                                            tabindex="-1"></iframe>
                                    </div>
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <div class="welcome-layout">
                            <div>
                                @php $welcomeLimits =
                                \App\Http\Controllers\Client\BirthdayCardController::WELCOME_LIMITS; @endphp
                                <div class="form-group">
            ...
            <div class="step-panel" id="panel4">
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <div class="step-nav">
                            <button class="btn-prev" onclick="prevStep()">← Back</button>
                            <button class="btn-next" id="step4ContinueBtn" onclick="saveStep4AndContinue()">Continue
                                →</button>
                        </div>
                    </div>
                </div>
            ...
            <div class="step-panel" id="panel5">
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        @php $gift1GirlLimits = \App\Http\Controllers\Client\BirthdayCardController::GIFT1_GIRL_LIMITS;
                        @endphp
                        ...
                        <div class="form-group gift2-field-row gift1-girl-only" id="gift1MessageRow">
                            ...
                            <textarea id="gift1Message" rows="3" maxlength="{{ $gift1GirlLimits['message'] }}"
                        ...
                        <div class="step-nav">
                            <button class="btn-prev" onclick="prevStep()">← Back</button>
                            <button class="btn-next" id="step5ContinueBtn" onclick="saveStep5AndContinue()">Continue
                                →</button>
                        </div>
                    </div>
                </div>
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <div id="gift2BoySide">
                            ...
                            @php $gift2Limits = \App\Http\Controllers\Client\BirthdayCardController::GIFT2_LIMITS;
                            @endphp
                            <div class="form-row gift2-field-row" id="gift2NamesRow">
                                <div class="form-group">
                                    ...
                                    <input type="text" id="gift2NameFirst" placeholder="e.g. Emma"
                                        maxlength="{{ $gift2Limits['name_first'] }}" oninput="updateGift2LivePreview()">
                                </div>
                                <div class="form-group">
                                    <div class="field-head">
                                        <label>Name 2</label>
                            ...
                            <div class="form-group gift2-field-row" id="gift2SignedGroup">
                                ...
                                <input type="text" id="gift2Signed" placeholder="e.g. — always, E"
                                    maxlength="{{ $gift2Limits['signed'] }}" oninput="updateGift2LivePreview()">
                            </div>
                        </div>
                        ...
                        <div id="gift2GirlSide">
                            <div class="gift2-help-note" role="note">
                                ...
                                <span><strong>How will Gift 2 open?</strong> The recipient should tap the ribbon first,
                                    then tap the gift box three times. The <strong>ⓘ</strong> button in the preview
                                    repeats these steps.</span>
                            </div>
                            <div class="book-step-dots" id="gift2GirlDots">
                                @foreach ($gift2GirlSteps as $i => $beat)
                                <span onclick="goToGift2GirlStep({{ $i + 1 }})" title="{{ $beat['title'] }}"></span>
                                @endforeach
                            </div>
                            ...
                            <div class="book-page-panel" id="gift2GirlStep{{ $i + 1 }}">
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="image-slots book-photo-slots">
                                        <div class="image-slot" id="gift2GirlSlot{{ $field['index'] }}"
                                            ...
                                            <img class="slot-preview" id="gift2GirlPhotoPreview{{ $field['index'] }}"
                                                alt="">
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" id="{{ $inputId }}" maxlength="{{ $gift2GirlLimits[$key] }}"
                        ...
                        <div class="step-nav">
                            <button class="btn-prev" onclick="prevStep()">← Back</button>
                            <button class="btn-next" id="step6ContinueBtn" onclick="saveStep6AndContinue()">Continue
                                →</button>
                        </div>
                    </div>
                </div>
            ...
            'hint' => 'Fits the letter paper — up to ' . $gift3LetterLines . ' lines. Longer notes are set in a smaller
            hand automatically.',
            ],
            ...
            <div class="step-panel" id="panel7">
                <div class="card">
                    ...
                    <div class="card-body">
                        <div class="variant-section visible" style="margin-top:0;">
                            ...
                            <div class="variant-grid" id="gift3ThemeGrid">
                                @for ($n = 1; $n <= 4; $n++) <div class="variant-choice" id="gift3Theme{{ $n }}"
                    ...
                    <div class="gift3-field-row" id="gift3BoySide">
                        ...
                        <div class="book-page-panel" id="bookPage{{ $i + 1 }}">
                            ...
                            <div class="form-group">
                                ...
                                <div class="image-slots book-photo-slots">
                                    <div class="image-slot" id="gift3Slot{{ $field['index'] }}"
                                        ...
                                        <img class="slot-preview" id="gift3PhotoPreview{{ $field['index'] }}" alt="">
                                        <span class="slot-plus">+<span>{{ $field['label'] }}</span></span>
                                    </div>
                                </div>
                                ...
                                <div id="dreamRows">
                                    ...
                                    <div class="checklist-row dream-row" id="dreamRow{{ $n }}">
                                        <input type="text" id="gift3_dream{{ $n }}"
                                            maxlength="{{ $gift3Limits['dream' . $n] }}" value="{{ $dream['text'] }}"
                                        ...
                                        <button type="button" class="dream-remove" onclick="removeDream({{ $n }})"
                                            title="Remove this dream" aria-label="Remove dream {{ $n }}">✕</button>
                                    </div>
                                    @endforeach
                                </div>
                                <button type="button" class="dream-add" id="dreamAddBtn" onclick="addDream()">+ Add a
                                    dream</button>
                            ...
                            <div class="form-group">
                                ...
                                <textarea id="{{ $inputId }}" rows="{{ $field['rows'] ?? 5 }}" maxlength="{{ $limit }}"
                        ...
                        <div class="book-page-nav">
                            <button class="btn-prev" id="gift3PrevPageBtn" onclick="goToBookPage(currentBookPage - 1)">←
                    ...
                    $g3GirlLetterLines =
                    \App\Http\Controllers\Client\BirthdayCardController::GIFT3_GIRL_LETTER_MAX_LINES;
                    ...
                    'hint' => 'Optional. With a screenshot the card shows it; without one the three lines below are
                    drawn as the chat.'],
                    ...
                    'default' => "Thank you for every smile.\n\nEvery photo here reminds me how lucky I am to have
                        ...
                        <div class="book-page-panel" id="gift3GirlStep{{ $i + 1 }}">
                            ...
                            <div class="form-group">
                                ...
                                <div class="image-slots book-photo-slots">
                                    <div class="image-slot" id="gift3GirlSlot{{ $field['index'] }}"
                                        onclick="document.getElementById('gift3PhotoInput{{ $field['index'] }}').click()">
                                        <img class="slot-preview" id="gift3GirlPhotoPreview{{ $field['index'] }}"
                                            alt="">
                                        <span class="slot-plus">+<span>{{ $field['label'] }}</span></span>
                                    </div>
                                </div>
                            ...
                            <div class="form-group">
                                ...
                                <textarea id="{{ $inputId }}" rows="{{ $field['rows'] ?? 4 }}"
                                    maxlength="{{ $g3GirlLimits[$key] }}" @isset($field['maxLines'])
                                    data-max-lines="{{ $field['maxLines'] }}" @endisset
                                    oninput="updateGift3LivePreview()">{{ $field['default'] }}</textarea>
                                @isset($field['hint'])
                                <p class="field-hint">{{ $field['hint'] }}</p>
                                @endisset
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" id="{{ $inputId }}" maxlength="{{ $g3GirlLimits[$key] }}"
                                    value="{{ $field['default'] }}" placeholder="{{ $field['default'] }}"
                                    oninput="updateGift3LivePreview()">
                            </div>
                            @endif
            ...
            'letter' => "Happy birthday, my love.\n\nWe reached the last page, but this isn't really the end — it's just
            where the words stop and the feeling keeps going.\n\nThank you for being exactly who you are — for every
            laugh, every little moment, every reason you give me to smile.\n\nHere's to you, and to every birthday still
            ahead of us.\n\nI love you, always.",
            ...
            $endingDefaultLetter = "Happy birthday, my love.\n\nWe reached the last page, but this isn't really the end
            — it's just where the words stop and the feeling keeps going.\n\nThank you for being exactly who you are —
            for every laugh, every little moment, every reason you give me to smile.\n\nHere's to you, and to every
            birthday still ahead of us.\n\nI love you, always.";
            ...
            <div class="step-panel" id="panel8">
                <div class="card">
                        ...
                        <div id="endingBoySide">
                            <div class="variant-section visible" style="margin-top:0;">
                                ...
                                <div class="variant-grid" id="endingThemeGrid">
                                    @for ($n = 1; $n <= 4; $n++) <div class="variant-choice" id="endingTheme{{ $n }}"
                    ...
                    <div class="step-nav">
                        <button class="btn-prev" onclick="prevStep()">← Back</button>
                        <button class="btn-next" id="step8ContinueBtn" onclick="saveStep8AndContinue()">Continue
                            →</button>
                    </div>
                </div>
            </div>
            ...
            <!-- ── STEP 9: Music ── -->
            @php $savedMusic = $card->music_data ?? []; @endphp
            <div class="step-panel" id="panel9">
                <div class="card">
                    <div class="card-header">
                        <div class="card-icon">🎵</div>
                        <div class="card-title">
                            <h3>Choose Story Music</h3>
                            <p>The song starts after unlock and stays with the story until the ending.</p>
                        </div>
                    </div>
                    <div class="card-body">
                        <div class="music-grid" id="musicGrid">
                            ...
                            <div class="music-choice" data-track-id="{{ $track->id }}"
                                data-track-url="{{ $track->url }}" onclick="selectLibraryMusic({{ $track->id }}, this)">
                            ...
                            @endforeach
                            @empty
                            <p>No default songs are available yet. Ask Super Admin to add one.</p>
                            @endforelse
                        </div>
                        ...
                        <div class="music-clip" id="musicClip" hidden>
                            <div class="music-clip-head">
                                <div>
                                    <strong>Pick your two minutes</strong>
                                    <p class="field-hint">Slide the bar to the part of the song you want. Those two
                                        minutes are what plays, looping from the welcome screen to the ending.</p>
                                </div>
                                <span class="music-clip-badge" id="musicClipBadge">2:00</span>
                            </div>

                            <div class="music-clip-slider" id="musicClipSlider">
                                <div class="music-clip-rail"></div>
                                <input type="range" id="musicClipRange" min="0" max="0" step="0.5" value="0"
                                    aria-label="Which part of the song to play">
                            </div>

                            <div class="music-clip-readout">
                                <span id="musicClipFrom">0:00</span>
                                <button type="button" class="music-clip-play" id="musicClipPlayBtn"
                                    onclick="toggleClipPreview()">▶ Play</button>
                                <span id="musicClipTo">2:00</span>
                            </div>

                            <div class="music-clip-progress"><span id="musicClipProgress"></span></div>
                            <div class="music-clip-foot">
                                <p class="field-hint" id="musicClipState"></p>
                                <label class="music-clip-vol" title="Preview volume">
                                    <span aria-hidden="true">🔊</span>
                                    <input type="range" id="musicClipVolume" min="0" max="1" step="0.01" value="1"
                                        aria-label="Preview volume">
                                </label>
                            </div>
                            {{-- preload="auto" so the bytes around the chosen part are already
                                 there when Play is pressed, rather than the picker sitting silent
                        ...
                        <p class="step9-error" id="step9Error"
                            style="display:none;color:#dc2626;font-size:.82rem;margin-top:1rem;"></p>
                        <div class="step-nav"><button class="btn-prev" onclick="prevStep()">← Back</button><button
                                class="btn-next" id="step9ContinueBtn" onclick="saveStep9AndContinue()">Continue
                                →</button></div>
                    </div>
                </div>
            </div>

            <!-- ── STEP 10: QR Select — generate the link and the code ── -->
            @php
            ...
            @endphp
            <div class="step-panel" id="panel10">
                <div class="card">
                    <div class="card-header">
                        <div class="card-icon">▦</div>
                        <div class="card-title">
                    ...
                    <div class="card-body">
                        ...
                        <div id="qrBoySide">
                            <div class="variant-section visible">
                                ...
                                <div class="qr-grid" id="qrThemeGrid">
                                    @for ($n = 1; $n <= 4; $n++) <div class="variant-choice" id="qrTheme{{ $n }}"
                            ...
                            <p class="step10-error" id="step10Error"
                                style="display:none; color:#dc2626; font-size:0.82rem; margin-top:1rem;">
                                Please choose a QR design to continue.</p>
                        </div>

                        {{-- The subscription gate. A card can be built and edited
                             freely; turning it into a shareable QR is what needs
                             an approved plan. --}}
                        @unless ($hasSubscription)
                            <div class="sub-gate" id="subGate">
                                <div class="sub-gate-head">
                                    <span class="sub-gate-icon">🔒</span>
                                    <div>
                                        <h4>Subscription required</h4>
                                        <p>Your card is saved. To generate its link and QR code, you need an
                                            active subscription.</p>
                                    </div>
                                </div>

                                @if ($pendingRequest)
                                    <div class="sub-gate-pending">
                                        ⏳ You requested
                                        <strong>{{ \App\Support\SubscriptionPlans::label($pendingRequest->plan_amount) }}</strong>
                                        on {{ $pendingRequest->created_at->format('d M Y') }}. Waiting for admin
                        ...
                        <div class="draft-before-qr">
                            <div>
                                <strong>Not ready to generate?</strong>
                                <p>Save this card as a draft and come back to it later — it will reopen right
                                    here.</p>
                            </div>
                            <button class="draft-save-btn solid" onclick="openDraftModal()">💾 Save as Draft</button>
                        </div>
    ...
    <!-- ─── Save as Draft: name the card ─── -->
    <div class="draft-modal" id="draftModal">
        <div class="draft-modal-box">
            <h3>Save as Draft</h3>
            <p class="draft-modal-sub">Give this card a name so you can find it on your dashboard.</p>

            <label class="draft-label">Card Label / Name</label>
            <input type="text" id="draftTitleInput" maxlength="80" placeholder="e.g. Ayesha's 21st Birthday"
                value="{{ $card->title }}" oninput="onDraftTitleInput()">
            <div class="draft-count"><span id="draftTitleCount">0</span>/80</div>

            <p class="draft-modal-msg" id="draftModalMsg"></p>

            <div class="draft-modal-actions">
                <button class="draft-btn ghost" onclick="closeDraftModal()">Cancel</button>
                <button class="draft-btn" id="draftSaveBtn" onclick="saveDraft()">Save Draft</button>
            </div>
        </div>
    </div>
    ...
    // How far this card had got. Reopening a draft resumes here rather than
    // sending the client back through steps they already finished. A card
    // saved at the end comes back on the QR step.
    const SAVED_STEP = Math.min(10, Math.max(1, @json((int) ($card->current_step ?: 1))));
    ...
    const CARD_STEP10_URL = @json(route('client.card.step10'));
    const CSRF_TOKEN = @json(csrf_token());

    // Every step save is tagged with the card this tab has open, so two tabs
    // editing two different cards can never write into each other. The server
    // prefers this header over anything it has in the session.
    const ACTIVE_CARD_ID = @json($card->id);
    const SUBSCRIPTION_REQUEST_URL = @json(route('client.subscription.request'));
    const HAS_SUBSCRIPTION = @json($hasSubscription);
    const CARDS_URL = @json(route('client.cards'));
    const CARD_DRAFT_URL = @json(route('client.card.draft'));

    (function () {
        const nativeFetch = window.fetch.bind(window);
        window.fetch = function (input, init) {
            const url = typeof input === 'string' ? input : (input && input.url) || '';
            if (ACTIVE_CARD_ID && url.indexOf('/client/card') !== -1) {
                init = init || {};
                const headers = new Headers(init.headers || {});
                headers.set('X-Card-Id', ACTIVE_CARD_ID);
    ...
    let step2ImageUrl = @json($card ? ($card->profile_image_path ? \Illuminate\Support\Facades\Storage::url($card->profile_image_path) : null) : null);
    ...
    const TOTAL_GIFT2_GIRL_STEPS = {{ $gift2GirlStepCount ?? 5 }};
    ...
    const TOTAL_GIFT3_GIRL_STEPS = {{ $g3GirlBeatCount ?? 9 }};
    let currentGift3GirlStep = 1;

    // Clips are uploads like the photos, just much larger ones.
    const GIFT3_VIDEO_MAX_KB = {{ \App\Http\Controllers\Client\BirthdayCardController::GIFT3_GIRL_VIDEO_MAX_KB }};
    ...
    function goToStep(n) {
        // Nothing should still be playing out of a step the client has left.
        stopClipPreview();
    ...
    // ── Step 8 / Step 9 / Step 10: ending, music and QR ─────
    ...
    let selectedMusicSource = @json($savedMusic['source'] ?? null);
    let selectedMusicTrackId = @json($savedMusic['track_id'] ?? null);
    let selectedMusicPath = @json($savedMusic['path'] ?? null);
    let selectedMusicUrl = @json(isset($savedMusic['path']) ? \Illuminate\Support\Facades\Storage::url($savedMusic['path']) : null);
    // The minute of the song the story plays, in seconds. Both null means the
    // whole track — a song shorter than a minute, or a card saved before the
    // clip picker existed.
    let musicClipStart = @json($savedMusic['trim_start'] ?? null);
    let musicClipEnd = @json($savedMusic['trim_end'] ?? null);
    let musicClipDuration = 0;
    let musicClipPlaying = false;
    const MUSIC_CLIP_SECONDS = @json(\App\Http\Controllers\Client\BirthdayCardController::MUSIC_CLIP_SECONDS);
    ...
    // ── Step 9: music selection and the clip picker ─────────
    function selectLibraryMusic(trackId, element) {
        // A different song opens on its own opening stretch; re-picking the
        // one already chosen keeps the window the client settled on.
        ...
        document.querySelectorAll('.music-choice').forEach(choice => choice.classList.remove('selected'));
        ...
        openClipPicker(element.dataset.trackUrl);
    }

    function restoreMusicSelection() {
        if (selectedMusicSource === 'library' && selectedMusicTrackId) {
            const choice = document.querySelector('[data-track-id="' + selectedMusicTrackId + '"]');
            if (choice) {
                choice.classList.add('selected');
                openClipPicker(choice.dataset.trackUrl);
            }
        }
    }
    ...
    /** How long the clip is for this song — the fixed length, or the whole of a shorter one. */
    function clipLength() {
        return Math.min(MUSIC_CLIP_SECONDS, musicClipDuration || MUSIC_CLIP_SECONDS);
    }

    /**
     * Show the clip picker for a track.
     *
     * The window can't be placed until the song's length is known, and that
    ...
    function openClipPicker(url) {
        ...
        // readyState is checked as well as the event: coming back to this step
        // with the same song loaded, loadedmetadata will never fire again.
        if (audio.readyState >= 1) clipPickerReady();
        else audio.addEventListener('loadedmetadata', clipPickerReady, { once: true });
    }

    function clipPickerReady() {
        const audio = document.getElementById('musicClipAudio');
        const duration = audio.duration;
        const range = document.getElementById('musicClipRange');

        if (!isFinite(duration) || duration <= 0) {
            document.getElementById('musicClipState').textContent =
                'This song’s length could not be read, so the whole track will play.';
            musicClipStart = null;
            musicClipEnd = null;
            return;
        }

        musicClipDuration = duration;

        // The handle travels the part of the song the window can't cover, so a
        // song no longer than the clip has nothing to slide and plays whole.
        const travel = Math.max(0, duration - MUSIC_CLIP_SECONDS);
        range.min = 0;
        range.max = travel.toFixed(2);
        range.disabled = travel <= 0;

        // A saved window is kept as long as it still fits this track.
        let from = Number(musicClipStart);
        if (!(musicClipStart !== null && isFinite(from) && from >= 0 && from <= travel + 0.5)) {
            from = 0;
        }
        range.value = Math.min(from, travel);

        sizeClipWindow();
        document.getElementById('musicClipPlayBtn').disabled = false;
        syncClipPicker();
    }

    /**
     * Widen the slider's thumb to the share of the track the clip covers.
     *
     * That is what makes the control read as a window rather than a point: the
     * thumb is the selection, and a range input's thumb already travels
     * exactly the rail minus its own width — the same distance the window has
     * to move to sweep the song.
     */
    function sizeClipWindow() {
        const slider = document.getElementById('musicClipSlider');
        if (!slider || !musicClipDuration) return;
        const width = slider.clientWidth;
        const share = Math.min(1, clipLength() / musicClipDuration);
        slider.style.setProperty('--clip-window', Math.max(28, Math.round(width * share)) + 'px');
    }

    /** Settle the window and repaint everything that reads from it. */
    function syncClipPicker() {
        const range = document.getElementById('musicClipRange');
        const duration = musicClipDuration;
        const length = clipLength();
        const from = Math.min(Number(range.value), Math.max(0, duration - length));
        const to = Math.min(duration, from + length);

        // A song no longer than the clip has nothing to pick out of it; that
        // is stored as no clip at all, and the file plays as it is.
        const whole = duration <= MUSIC_CLIP_SECONDS + 0.25;
        musicClipStart = whole ? null : Number(from.toFixed(2));
        musicClipEnd = whole ? null : Number(to.toFixed(2));

        document.getElementById('musicClipFrom').textContent = formatClock(from);
        document.getElementById('musicClipTo').textContent = formatClock(to);
        document.getElementById('musicClipBadge').textContent = formatClock(to - from);
        document.getElementById('musicClipState').textContent = whole
            ? 'This song is ' + formatClock(duration) + ' long, so all of it plays, on a loop.'
            : formatClock(from) + ' – ' + formatClock(to) + ' of ' + formatClock(duration) + ', on a loop.';
    }

    function toggleClipPreview() {
        if (musicClipPlaying) stopClipPreview();
        else startClipPreview();
    }

    /** Play the chosen part, from its first second. */
    function startClipPreview() {
        const audio = document.getElementById('musicClipAudio');
        if (!audio) return;

        // The per-song players in the grid are a separate thing; two tracks at
        // once helps nobody.
        document.querySelectorAll('.music-choice audio').forEach(el => el.pause());

        // A preview nobody can hear is the commonest way for this to look
        // broken, so the two things that silence it are set every time rather
        // than assumed: an element muted by a stray earlier state, and a volume
        // left at zero on the slider.
        audio.muted = false;
        audio.volume = Number(document.getElementById('musicClipVolume').value) || 1;

        try { audio.currentTime = Number(document.getElementById('musicClipRange').value); } catch (e) {}

        audio.play().then(() => {
            musicClipPlaying = true;
            document.getElementById('musicClipPlayBtn').textContent = '❚❚ Stop';
        }).catch(() => {
            // Swallowing this is what made a silent preview so hard to place.
            stopClipPreview();
            document.getElementById('musicClipState').textContent =
                'Your browser would not start the preview — press Play once more.';
        });
    }
    ...
    function stopClipPreview() {
        ...
        const button = document.getElementById('musicClipPlayBtn');
        if (button) button.textContent = '▶ Play';
        const progress = document.getElementById('musicClipProgress');
    ...
    (function () {
        ...
        // The preview plays the clip the way the story will: round and round
        // the window, so what the client hears is what the recipient gets.
        audio.addEventListener('timeupdate', () => {
            const from = Number(range.value);
            const to = from + clipLength();
            if (audio.currentTime >= to - 0.05 || audio.currentTime < from - 0.25) {
                try { audio.currentTime = from; } catch (e) {}
            }
            const done = (audio.currentTime - from) / clipLength();
            document.getElementById('musicClipProgress').style.width =
                Math.min(100, Math.max(0, done * 100)) + '%';
        });

        // Dragging scrubs: the part under the handle is what you hear, the
        // moment you move it there.
        range.addEventListener('input', () => {
            syncClipPicker();
            if (musicClipPlaying) {
                try { audio.currentTime = Number(range.value); } catch (e) {}
            }
        });

        // Letting go starts the clip if nothing was playing, so choosing a
        // part and hearing it are the same gesture.
        range.addEventListener('change', () => {
            syncClipPicker();
            startClipPreview();
        });

        // Seeking into the middle of a song is a network round trip. Saying so
        // beats a Play button that looks stuck.
        audio.addEventListener('waiting', () => {
            if (musicClipPlaying) {
                document.getElementById('musicClipState').textContent = 'Buffering…';
            }
        });

        audio.addEventListener('playing', syncClipPicker);

        audio.addEventListener('error', () => {
            stopClipPreview();
            document.getElementById('musicClipPlayBtn').disabled = true;
            document.getElementById('musicClipState').textContent =
                'This song could not be loaded. Ask Super Admin to check the file.';
        });

        const volume = document.getElementById('musicClipVolume');
        volume.addEventListener('input', () => {
            audio.muted = false;
            audio.volume = Number(volume.value);
        });

        window.addEventListener('resize', sizeClipWindow);
    })();

    function saveStep9AndContinue() {
        const errorEl = document.getElementById('step9Error');
        if (selectedMusicSource !== 'library' || !selectedMusicTrackId) {
            errorEl.textContent = 'Please choose a song from the Super Admin music library.';
            errorEl.style.display = 'block';
            return;
        }
        stopClipPreview();
        const btn = document.getElementById('step9ContinueBtn');
        const originalText = btn.textContent;
        btn.disabled = true;
        btn.textContent = 'Saving…';
        const body = new FormData();
        body.append('source', selectedMusicSource);
        if (selectedMusicTrackId) body.append('track_id', selectedMusicTrackId);
        ...
        fetch(CARD_STEP9_URL, {
            ...
            .then(({
                ...
                if (!ok) throw new Error(data.message || 'Could not save music.');
                nextStep();
            })
            .catch(error => {
    ...
    function selectQrTheme(n) {
        ...
        document.getElementById('step10Error').style.display = 'none';
    }
    ...
    // Ask the admin for a plan. No payment step yet — the request is the
    // whole flow until an admin approves it.
    // ── Save as Draft ──────────────────────────────────────
    // The steps already persist as they go; this names the card and records
    // how far it got, so the dashboard can label it and Edit can reopen here.
    function openDraftModal() {
        const modal = document.getElementById('draftModal');
        modal.classList.add('open');
        document.getElementById('draftModalMsg').className = 'draft-modal-msg';
        onDraftTitleInput();
        const input = document.getElementById('draftTitleInput');
        input.focus();
        input.select();
    }
    ...
    function onDraftTitleInput() {
        const input = document.getElementById('draftTitleInput');
        document.getElementById('draftTitleCount').textContent = input.value.length;
    }

    function saveDraft() {
        const btn = document.getElementById('draftSaveBtn');
        const msg = document.getElementById('draftModalMsg');
        const title = document.getElementById('draftTitleInput').value.trim();

        if (!title) {
            msg.textContent = 'Please give this card a name.';
            msg.className = 'draft-modal-msg bad';
            document.getElementById('draftTitleInput').focus();
            return;
        }
        ...
        body.append('current_step', currentStep);

        fetch(CARD_DRAFT_URL, {
                method: 'POST',
                headers: {
                    'Accept': 'application/json',
                    'X-CSRF-TOKEN': CSRF_TOKEN,
            ...
            .then(({ ok, data }) => {
                ...
                msg.textContent = 'Draft saved. Taking you to your dashboard…';
                msg.className = 'draft-modal-msg ok';
                btn.textContent = 'Saved ✓';
                // The card title shows in the topbar, so reflect it at once.
                document.querySelector('.topbar-left h2').textContent = data.title;
                setTimeout(() => window.location.href = data.redirect || CARDS_URL, 900);
            })
            .catch(() => {
                msg.textContent = 'Could not save the draft. Please try again.';
                msg.className = 'draft-modal-msg bad';
                btn.disabled = false;
    ...
    function generateCard() {
        const errorEl = document.getElementById('step10Error');

        if (!HAS_SUBSCRIPTION) {
            errorEl.textContent = 'An active subscription is required to generate the QR code.';
            errorEl.style.display = 'block';
        ...
        fetch(CARD_STEP10_URL, {
            ...
            .then(res => res.json().then(data => {
                if (!res.ok) {
                    const err = new Error(data.message || 'Generate failed');
                    err.handled = data.reason === 'subscription_required';
                    throw err;
                }
                return data;
            }))
            ...
            .catch(err => {
                errorEl.textContent = err && err.handled
                    ? err.message
    ...
    window.addEventListener('load', function() {
        ...
        // selectTheme() resets selectedVariant, so the saved design is held
        // aside first — reading it back afterwards always saw null, which is
        // why a reopened card kept asking for its design again.
        ...
        giftPhotoUrls.gift2.forEach((url, i) => {
            ...
            const slot = document.getElementById('gift2Slot' + i) || document.getElementById(
                'gift2Slot' + i +
                'Wrap');
            slot.classList.add('filled');
        ...
        // ── Resume ──
        // Everything above has restored this card's saved values into the
        // form. Only now is it safe to jump to where the client left off:
        // goToStep() fires each step's own loaders, and those need the
        // restored state to already be in place.
        resumeAtSavedStep();
    });

    /**
     * Reopen the wizard at the furthest step this card reached, marking the
     * ones before it as done so the sidebar reads correctly. A card that was
     * saved at the end lands on the QR step, ready to generate.
     */
    function resumeAtSavedStep() {
        if (SAVED_STEP <= 1) return;

        const items = document.querySelectorAll('.step-item');
        for (let i = 1; i < SAVED_STEP; i++) {
